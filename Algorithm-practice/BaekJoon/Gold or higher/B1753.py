...
2. 한 정점의 간선에 대해 우선순위 큐에 [비용, 정점] 정보를 넣고 빼는 과정: O(logE)
최악의 경우는 모든 간선을 검사할 때 마다 비용 리스트가 갱신되고, 우선순위 큐에 정보가 저장되는 경우이다. 
E개의 간선을 검사할 때 마다 우선순위 큐를 유지해야 하므로 이 과정의 시간 복잡도는 O(logE)이다.

위 두 과정을 거칠 경우 다익스트라 알고리즘의 총 시간 복잡도는 O(ElogE) 가 된다.

중복 원소를 우선 순위 큐에 넣지 않도록 하여 O(VlogV)O(VlogV)에 수행 가능하다고 한다. 
그러나 실제로는 구현의 복잡도나 오버헤드때문에 잘 쓰지 않는다고 한다.
